//! Pure, storage-agnostic 4-D spatio-temporal indexer.
//!
//! This crate contains only computation — Morton encoding, coordinate
//! normalization, key generation, and range planning. It has no I/O and no
//! async runtime, so it compiles to `wasm32-unknown-unknown` for use in the
//! browser (paired with IndexedDB) as well as on native targets.
